...
while True: 
    ...
    if "a" in selection:
        models = net_connect.send_command("show platform software ndbman r0 models")
        ...
        refactored_uri = tdl_uri.replace('?', '/')
        output = net_connect.send_command("show platform software ndbman r0 database hi" + " " + refactored_uri)
        filter = input("Enter filter(hit enter for all):")
        print()
        if filter == "":
            print(output)
        ...
        refactored_column = column_uri.replace('?', '/')
        output = net_connect.send_command("show platform software ndbman r0 database ta" + " " + refactored_column)
        print (output)
    elif "d" in selection:
        record_uri = input ("Record uri:")
        refactored_record = record_uri.replace('?', '/')
        output = net_connect.send_command("show platform software ndbman r0 database re" + " " + refactored_record)
        print (output)
    elif "e" in selection:
        record_uri = input ("Get N Recursive uri:")
        refactored_record = record_uri.replace('?', '/')
        output = net_connect.send_command("show platform software ndbman r0 database get_n_recursive" + " " + refactored_record)
        print (output)
    else:
